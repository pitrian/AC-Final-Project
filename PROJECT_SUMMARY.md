...
- [x] `createPlan`: valid plan, invalid APR (0), disabled by default
- [x] `updatePlan`: update APR affects new deposits only
- [x] `enable/disablePlan`: disabled plan rejects deposits

**Test Cases - Open Deposit:**
- [x] `openDeposit`: happy path, thành công
- [x] `openDeposit`: below minDeposit - should revert
- [x] `openDeposit`: above maxDeposit - should revert
- [x] `openDeposit`: disabled plan - should revert
- [x] `openDeposit`: NFT được mint đúng

**Definition of Done:**
- [x] All test cases pass
- [x] Clear error messages cho từng revert case
...
- [x] `withdraw`: correct interest calculation
- [x] `withdraw`: too early - should revert or apply early penalty
- [x] `withdraw`: already withdrawn - should revert
- [x] `withdraw`: insufficient vault - should revert

**Test Cases - Early Withdrawal:**
- [x] `earlyWithdraw`: correct penalty calculation
- [x] `earlyWithdraw`: interest = 0
- [x] `earlyWithdraw`: penalty transferred to feeReceiver
- [x] `earlyWithdraw`: NFT burned

**Test Cases - Manual Renew:**
- [x] `renewDeposit`: correct new principal (principal + interest)
- [x] `renewDeposit`: status update (old NFT marked)
- [x] `renewDeposit`: new NFT có APR mới (market rate)
- [x] `renewDeposit`: before maturity - should revert
- [x] `renewDeposit`: wrong plan - should revert

**Definition of Done:**
- [x] All withdraw/renew tests pass
- [x] Math calculations chính xác
...
- [x] `autoRenewDeposit`: before grace period (3 days) - should revert
- [x] `autoRenewDeposit`: after grace period - success
- [x] `autoRenewDeposit`: APR locked (NOT market rate)
- [x] `autoRenewDeposit`: anyone can trigger (not just owner)
- [x] `autoRenewDeposit`: new principal = old + interest
- [x] `autoRenewDeposit`: old NFT marked as AutoRenewed

**Test Cases - Vault:**
- [x] `fundVault`: admin fund vault
- [x] `withdrawVault`: admin withdraw (within safe limit)
- [x] `withdrawVault`: insufficient vault for interest payout - revert

**Test Cases - Pause:**
- [x] `pause`: withdraw blocked when paused
- [x] `pause`: renew blocked when paused
- [x] `unpause`: restore functionality
...
**Tasks:**
- [x] Chạy `npx hardhat coverage` hoặc tương đương
- [x] Analyze uncovered lines
- [x] Add missing test cases
- [x] Fix logic bugs discovered
- [x] Refactor tests nếu cần
...
**Tasks:**
- [x] Tạo React project (Vite hoặc CRA)
- [x] Install dependencies: ethers, wagmi (optional)
- [x] Cấu hình MetaMask connection
- [x] Create contract instances (use ethers.js)
- [x] Create hooks: `useWallet`, `useContracts`
- [x] Setup environment variables (.env)

**Definition of Done:**
- [x] Frontend build thành công
- [x] Kết nối MetaMask được
- [x] Contract instances có thể gọi
...
**Features:**
- [x] **View Plans**: Hiển thị danh sách available plans
- [x] **Open Deposit**: Form để gửi tiền (chọn plan, nhập amount)
- [x] **View Deposits**: Hiển thị user's active deposits (từ NFT)
- [x] **Withdraw**: Button để rút tiền (đúng hạn/sớm)
- [x] **Renew**: Button để gia hạn (manual)

**UI Requirements:**
- [x] Connect/Disconnect MetaMask button
- [x] Display connected address
- [x] Display USDC balance
- [x] Show plan details (tenor, APR, min/max)
- [x] Show deposit details (principal, maturity, APR)
- [x] Action buttons (Deposit, Withdraw, Renew)
- [x] Loading states và error handling

**Definition of Done:**
- [x] Tất cả features hoạt động
- [x] Giao diện usable
- [x] Integration với contracts đúng
...
2. Import account từ Hardhat node (private key trong terminal)
3. Xin mint USDC từ Hardhat console: `await mockUSDC.mint(address, amount)`
4. Test các tính năng: View Plans, Open Deposit, Withdraw, Renew
